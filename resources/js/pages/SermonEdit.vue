...
<!--          <p class="">New Sermon</p>-->
          <p class="content-heading" >Edit Sermon</p>
          <div class="content-wrapper">
            <v-row>
              ...
              >
                <v-btn :disabled="sermonsUpdateStatus===1" dark @click="updateSermon">Update</v-btn>
              </v-col>
            </v-row>
          </div>
        </v-container>
...
export default {
  props:['slug'],
    ...
    computed: {
      sermon(){
        return (this.$store.getters.getSermon).sermon
      },
      sermonLoadStatus(){
      ...
      sermonsUpdateStatus(){
        return this.$store.getters.getSermonsUpdateStatus
      },
      validation(){
      ...
      published_at(){
        return this.getTimestamp(this.date)
      }

    },
    watch: {
      sermonLoadStatus:function () {
        switch (this.sermonLoadStatus) {
          case 1:
            break;
          case 2:
            this.title=this.sermon.title
            this.subtitle=this.sermon.subtitle
            this.video_url=this.sermon.video_url
            this.body=this.sermon.body
            this.author_id=this.sermon.author.id
            this.series_id=this.sermon.series?this.sermon.series.id:null
            this.date=new Date((this.sermon.published_at)*1000).toISOString().substr(0,10)
            break;
          case 3:
            break;
          case 4:
      ...
      sermonsUpdateStatus:function () {
        switch (this.sermonsUpdateStatus) {
          case 1:
            this.snackbarMessage="Updating sermon";
            break;
          case 2:
    ...
    methods:{
      ...
      updateSermon(){
        if(!this.validation)
          ...
          this.$store.dispatch("SermonUpdate",{
            slug:this.slug,
            title:this.title,
